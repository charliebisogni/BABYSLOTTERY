...
export default function AccesoResultadosPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!bebe.calculo_realizado) {
        setErrors({ general: "Aún no hay resultados disponibles para este bebé" })
        setIsSubmitting(false)
        return
      }
      ...
      // Guardar el ID del bebé en sessionStorage para usarlo en la página de resultados
      sessionStorage.setItem(
        "bebeResultadosAutorizado",
        JSON.stringify({
          id: bebe.id,
      ...
      router.push(`/evento/${bebe.id}/resultados`)
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <Card className="max-w-md mx-auto">
        <CardHeader>
          <CardTitle className="text-center text-2xl">Acceso a Resultados 🏆</CardTitle>
          <CardDescription className="text-center">
            Ingresa el identificador del bebé y la contraseña proporcionada por los padres para ver los resultados
          </CardDescription>
        </CardHeader>
